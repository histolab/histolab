...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
    ...
    summary_font = ImageFont.truetype(
        SUMMARY_TITLE_FONT_PATH, size=SUMMARY_TITLE_TEXT_SIZE
    )
...
def summary_and_tiles(
    slide_num, display=True, save_summary=False, save_data=True, save_top_tiles=True
):
    """
    ...
    generate_tile_summaries(
        tile_sum, np_img, display=display, save_summary=save_summary
    )
