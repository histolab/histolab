...
First, we need to load some modules and an example `Slide <api/slide.html#src.histolab.slide.Slide>`_; here we will consider the `Kidney <api/data.html#src.histolab.data.ihc_kidney>`_ WSI available in the `data <api/data.html#data>`_ module.
...
From our `Slide <api/slide.html#src.histolab.slide.Slide>`_ object we can now retrieve a binary mask considering specific regions of the tissue. Notice that available masks are defined in the `masks <api/masks.html#src.histolab.masks>`_ module.

As a diagnostic check to visualize the mask, we can call the `locate mask <api/slide.html#src.histolab.slide.Slide.locate_mask>`_ method on the Slide, which outlines the boundaries of the selected mask on the slide's thumbnail.


TissueMask
----------

If we want to account for all the tissue detected on the slide, the `TissueMask <api/masks.html#src.histolab.masks.TissueMask>`_ is what we need:
...
The `BiggestTissueBoxMask <api/masks.html#src.histolab.masks.BiggestTissueBoxMask>`_ keeps only the largest connected component of the tissue, and returns the bounding box including that region:
...
It is also possible to define a custom binary mask by subclassing the `BinaryMask <api/masks.html#src.histolab.masks.BinaryMask>`_ object.
...
We can finally pass our mask to the `extract <api/tiler.html#src.histolab.tiler.RandomTiler.extract>`_ method of our `Tiler <api/tiler.html#src.histolab.tiler>`_ object, and visualize the location of the extracted tiles:
...
.. note::
    The `BiggestTissueBoxMask <api/masks.html#src.histolab.masks.BiggestTissueBoxMask>`_ is considered as default binary mask.
