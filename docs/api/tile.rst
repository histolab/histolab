Tile
====================================
The ``tile`` module contains the ``Tile`` class to manage a rectangular region cropped from a `Slide <slide.html#src.histolab.slide.Slide>`_. A ``Tile`` object is described by (i) its extraction coordinates at native magnification (corresponding to level 0 in ``OpenSlide``), (ii) the level of extraction, (iii) the actual image, stored as a ``PIL Image``. A ``Tile`` object will be created internally during the tile extraction process.
