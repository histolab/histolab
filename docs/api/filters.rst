...
Despite being formally correct, the above implementation in neither intuitive or economic (in terms of memory used and lines of code). The use of a `Compose <filters/image_filters.html#histolab.filters.image_filters.Compose>`_ object leads to a more compact implementation:
...
.. note::
    Although ``not_composed_filters`` and ``composed_filters`` functions return the same result, the use of `Compose <filters/image_filters.html#histolab.filters.image_filters.Compose>`_ avoids storing intermediate results and wasting memory in case of very large input image. For example, the peak RAM usage for an image of size 19,394px x 6,136px (84.9 MB) is ~2600 MB when using ``composed_filters`` in contrast to the ~3200 MB allocated for the ``not_composed_filters`` function.
